...
public class PerfCommandParser implements Parser<Command> {
    ...
    public Command parse(String args) throws ParseException {
        ...
        if (trimmed.startsWith("-a")) {
            ...
            String sid = require(map, PREFIX_STUDENTID, "Missing student id: use s/STUDENT_ID");
            ...
            return new PerfAddCommand(sid, date, note);

        } else if (trimmed.startsWith("-v")) {
            // perf -v s/STUDENT_ID
            String sid = require(map, PREFIX_STUDENTID, "Missing student id: use s/STUDENT_ID");
            validateStudentId(sid);
            return new PerfViewCommand(sid);
            ...
            return new PerfEditCommand(sid, oneBasedIndex, note);
            ...
            return new PerfDeleteCommand(sid, oneBasedIndex);
        }
    ...
    private static void validateStudentId(String sid) throws ParseException {
        if (!sid.matches("\\d{4}")) {
            throw new ParseException("Invalid student id. Please enter a valid number within the range of existing students.");
        }
    }
